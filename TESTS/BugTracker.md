...
| BUG-12 | `package.json` | Packaging | The declared `"unity": "6000.5"` floor blocks a clean UPM git-package install for any project on an older Editor — confirmed by `astro aces` (Unity 6000.3.21f1), forced into hand-vendoring the `Runtime` folder instead, which itself risks orphaning existing script references into "Missing (Mono Script)" unless old `.meta` GUIDs are manually preserved (a real, sharp-edged workaround, not a normal install path). | **Open — floor looks arbitrary, not yet safe to just lower.** Confirmed via `git log -p -- package.json`: `"unity": "6000.5"` has been unchanged since the very first commit, almost certainly just matching whatever Editor happened to be installed at project creation, never a deliberate minimum-API decision. Grepped all of `Runtime/` for anything version-gated or exotic (`Awaitable`, render-pipeline APIs, `UNITY_6000`/`UNITY_2023` symbols, etc.) — nothing found. That's real supporting evidence the floor is over-conservative, but not proof a lower number is actually safe — would need an actual compile/test pass against an older Editor (e.g. 6000.3, or further back) before changing the declared value. Lowering it blind and being wrong (silent incompatibility on some older API) would be worse than leaving it as-is. | 2026-08-22 |
...
- 2026-08-22 15:57 — First report from a new team, `astro aces`: real, successful use of
  `CustomCapture` closed a months-old flight-aim bug. Added BUG-12 (`package.json`'s `unity: 6000.5`
  floor is likely over-conservative, confirmed never deliberately set and nothing version-gated
  found in `Runtime/`, but not yet safe to lower without testing against an older Editor). Updated
  BUG-05 — the `Mouse.current` stretch goal now has real corroborating demand from a second
  independent team.
