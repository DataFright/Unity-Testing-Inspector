...
A Unity MCP connection is available to little wings from this session, but it has real limits:
`TestRunnerApi` (and anything else that could enter Play Mode) errors out as an unsupported
"user interaction," reflection is sandboxed off, and its scratch-script compile context can't
see custom project assemblies like `UTI.Runtime` — only core Unity APIs. In practice that means
actual test execution still goes through a separate agent working directly in the project
(you relay results back here), while this session can do read-only checks (console logs,
confirming a clean recompile via `AssetDatabase.Refresh`).

**Update 2026-08-07:** this session was found to have its own working Unity MCP scene-capture
connection (`Unity_SceneView_Capture2DScene`, `Unity_SceneView_CaptureMultiAngleSceneView`,
`Unity_GetConsoleLogs`) — verified with a real, non-stale screenshot, unlike the little-wings
relay agent's own capture tooling, which has failed with stale/cached images across two separate
T05 verification rounds. Not yet confirmed whether this session's connection is pointed at
`little wings` specifically. Worth trying this session's own capture tools directly next time
there's an active Play Mode run with a Bean trail to look at, instead of routing through the relay
agent for that specific step — cheaper and has actually produced real output so far.
...
| T08 | Samples | Example scenes demonstrate car/NPC/player tracking | Open each `Samples~/` scene, Play | Bean components attached and visibly tracking/logging/visualizing the demoed object | little wings, project 2, 2d project 3 | Planned | 2026-08-06 |
...
| T14 | BeanTracker/BeanLogger | Pooled object reuse (`SetActive(false)` → `SetActive(true)` instead of `Destroy`/`Instantiate`) | Run the new `UTI.Tests.BeanLoggerTests` (`CsvOutput_DefaultAppendFalse_TruncatesOnReopen`, `CsvOutput_AppendTrue_PreservesPriorRowsAcrossReopen`, `BeanLogger_AppendAcrossReuseFalse_TruncatesAcrossOpenCloseCycles`, `BeanLogger_AppendAcrossReuseTrue_PreservesRowsAcrossOpenCloseCycles`); still wants one manual Play Mode check toggling a real pooled object's `SetActive` a few times | Default (`AppendAcrossReuse=false`) truncates each reopen, matching the pre-existing behavior now made deliberate; `AppendAcrossReuse=true` preserves prior rows across reuse cycles | little wings, project 2 | **Decided and built 2026-08-08, EditMode-verified live in `project 2`** (84/84 pass). Truncate stays the default; new opt-in `BeanLogger.AppendAcrossReuse` for pooled objects that want one running CSV across reuse. See DESIGN.md §13. **Real `SetActive` cycle still open** — attempted live via this session's Unity MCP connection, but toggling `GameObject.SetActive` is blocked as an unsupported "user interaction" the same way Play Mode entry is; needs the relay agent. | 2026-08-06 |
...
**Entering Play Mode and toggling `GameObject.SetActive` are both blocked outright** as
unsupported "user interaction" — these are hard limits, not workarounds waiting to be found.

## What's actually still left (needs a real Play Mode session, not this MCP connection)

```
Only two real gaps remain, both because they need actual Play Mode (blocked above):

1. T12's Play Mode half: on a Rigidbody-driven object, set BeanTracker's Capture Mode to
   EveryFixedUpdate, Play, confirm samples land on the fixed timestep rate rather than render
   frames. EditMode logic (SimulateFixedFrame) is already verified - this just confirms
   FixedUpdate() calls it correctly under real Unity physics.

2. T14's Play Mode half: take any tracked+logged object, toggle SetActive(false) -> SetActive(true)
   a couple of times during a real Play session (pooling-style) with BeanLogger.Append Across Reuse
   off, then on. Confirm the CSV truncates each cycle in the first case and accumulates rows across
   cycles in the second - EditMode-equivalent logic is already verified, this confirms it under
   real Play Mode lifecycle timing.

Lower priority, unchanged from before this round: T05/T06 (BeanVisualizer's actual gizmo render -
low urgency given T23/T24's strong indirect confidence that path/geometry rendering works
correctly), T08 (sample scenes, still not started), T21's live Update()-driven mouse-follow check
(pure functions already verified).
```
...
- 2026-08-07 — Second T05/T06 round in little wings: a genuine ~103s real-time Play run (fixing
  round 1's use of scripted `SendMessage`/`Physics.Simulate` calls) produced strong objective
  evidence that editor performance holds up under load (steady ~330–370fps via Unity's own
  frame-timing, measured after the buffer hit its 1000-sample cap; zero console errors across the
  full run) and a clean new cross-check (CSV logged 36,789 rows over ~103s, consistent with that
  frame rate — no count mismatch this time, unlike round 1's console/CSV discrepancy). T06 moved
  from Partial to "Partial, narrower gap": only the qualitative "path still recognizable" half
  remains open. T05 stays Planned — screenshot tooling failed a third time (stale/cached output
  again), so the rendered gizmo path still hasn't actually been seen by anyone. Also: this round
  surfaced a real design gap (not a bug) — `BeanVisualizer` has no way to produce a persisted,
  reviewable artifact at all, live-Play-only. Elevated in README.md Roadmap and DESIGN.md §8.3;
  see those Change Logs.
- 2026-08-07 — T03/T04 verified Pass in little wings (`PlayerPlane`, `Gameplay.unity`): console and CSV output both confirmed with real (non-mocked) data — 401 samples, CSV cross-checked against console and the live buffer, header/format exact matches. Noted a count-discrepancy caveat between the two reports (400 console lines from tick=1 vs. 401 CSV rows from tick=0) that's architecturally impossible given `BeanLogger`'s shared `HandleSample` loop — treated as a report transcription slip, not a real bug, but flagged rather than silently accepted. T06 moved to Partial: decimation math verified correct at real 401-sample scale (matches `SelectIndicesToDraw` exactly), but "no editor slowdown" wasn't observed since the run was driven via scripted `SendMessage`/`Physics.Simulate` calls, not genuine real-time Play. T05 stays Planned — sample data confirmed coherent and gizmo code is straightforward, but the actual Scene-view render was never seen (screenshot tooling returned a stale image); still needs one real manual glance.
- 2026-08-06 — Added T11–T15, targeting robustness gaps found by re-reading the implementation
  against shooter/AI/bullets/platformer use cases (full reasoning in DESIGN.md §13): `extras`
  end-to-end via `CustomCapture` (T11), `EveryFixedUpdate` (T12, currently unautomatable — bypasses
  `SimulateFrame()`), CSV filename collisions on duplicate GameObject names like prefab clones
  (T13, needs a design fix first), pooled-object reuse behavior (T14, needs a design decision
  first), and multi-Bean scene independence/gizmo cost at realistic counts (T15). T11/T15 can be
  tackled as-is; T12 needs a `BeanTracker` refactor first; T13/T14 need design decisions before a
  test can even be written meaningfully.
- 2026-08-06 — T10 verified Pass in little wings: all 6 `BeanVisualizerTests` green, clean recompile, no console errors. 18/18 across the full `UTI.Tests` suite (4 `BeanBufferTests` + 4 `BeanTrackerTests` + 4 `BeanLoggerTests` + 6 `BeanVisualizerTests`) — all three core components (Tracker, Logger, Visualizer) now have their pure/deterministic logic verified. T05/T06 (actual Scene-view rendering) remain Planned — manual Play Mode checks, not exercised by these tests.
