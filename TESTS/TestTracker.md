...
Handed to a project with **zero prior UTI exposure**, specifically to get an independent, first-hand
...
1. Install UTI from the public GitHub URL (not a local file: path - we specifically want the real
   external-user experience this time): Window > Package Manager > + > "Add package from git URL":

   https://github.com/DataFright/Unity-Testing-Inspector.git

   Or edit Packages/manifest.json by hand:
   "com.uti.core": "https://github.com/DataFright/Unity-Testing-Inspector.git"
   plus a "testables": ["com.uti.core"] entry (required or Test Runner silently finds 0 tests).
   Confirm it resolves with zero console errors.

2. Run the Editor menu item UTI > Setup Project (Config + Docs). Confirm it creates
...
**Update 2026-08-09:** T05 was briefly marked Pass, then reverted back to Planned - see its row
above for the full, honest history.

Remaining, lower priority:

```
T05/T06 (BeanVisualizer's actual gizmo render - this session's own live attempts never reproduced
one; still genuinely unconfirmed by this session's standard), T08 (sample scenes, still not
started), T21's live Update()-driven mouse-follow check (pure functions already verified).
```
...
- 2026-08-09 11:58 — **Added T30: a fourth test project (`first project`), install from the real
  ...
  and handed off; `first project` is a fresh Unity URP template with no existing gameplay, so the
  prompt asks for one minimal movement test built purely to exercise UTI, not a real feature.
- 2026-08-09 11:50 — **T05 reverted from Pass back to Planned, per direct user correction.** The
  10:58 entry below marked T05 Pass based on two screenshots the user provided. Corrected: those
  screenshots were captured by the user, not generated by this session, and this session's own
  separate live attempts (real Play Mode, both `project 2` and `little wings`) never reproduced a
  visible gizmo line themselves. The user does not consider the screenshots' source independently
  verified against this session's own code/environment. A result this session could not reproduce,
  from a source it could not independently verify, doesn't meet this doc's own "Pass only after a
  real, verified report" standard - reverted honestly rather than left standing. T05's row keeps
  the full history, including the reverted Pass, for the record.
- 2026-08-09 10:58 — **T05 finally confirmed Pass — first real confirmation in this project's
  history, after five automated-capture attempts across two projects.** The user shared two real
  screen/window captures (not a synthetic MCP render tool) of `little wings`' live Play-Mode Scene
  view, taken by a separate session working directly in that project, showing `BeanVisualizer`'s
  path line rendering correctly near `PlayerPlane`. The line's magenta color initially raised a real
  concern (magenta is Unity's classic shader-fallback color) worth checking rather than dismissing -
  confirmed live that (1) `Shader.Find("Sprites/Default")` resolves fine under `little wings`'
  active URP pipeline, and (2) more decisively, `Gizmos.DrawLine` (what `BeanVisualizer` actually
  uses) has no `Shader`/`Material` dependency at all, so a shader-pipeline failure can't structurally
  apply to this code path - that risk is specific to `BeanSnapshotExporter`'s `LineRenderer`, a
  different component. Concluded the magenta is a deliberately-chosen high-contrast color choice,
  not a bug. This closes out the same investigation from earlier today (see the 10:19/10:42 entries
  below): confirmed there was no code regression to chase, and this real screen capture - as opposed
  to every automated capture tool tried, which consistently fails to render the Gizmo overlay layer
  at all - is what finally provided direct evidence. T05's own row keeps the full history of all
  four prior inconclusive attempts for the record.
- 2026-08-09 10:42 — **T05 detective work + fifth attempt: confirmed this was never actually
  working, in either test project, and narrowed the tooling gap further via a cross-project test.**
  At the user's request (they believed the gizmo line worked before multi-angle capture was added,
  but weren't certain), re-read every dated T05 entry across this file's history and confirmed via
  `git log -p --follow -- Runtime/BeanVisualizer.cs` that the file has exactly one commit total
  (the initial one) - it has never been edited, ruling out a code regression. T24 (multi-angle
  capture) only ever touched `BeanSnapshotExporter.cs`. The likely source of the belief: real PNGs
  in `little wings/UTI/BeanSnapshots/` (T16/T17), opened directly and confirmed to be
  `BeanSnapshotExporter`'s Camera-render output (filename matches `ResolveSnapshotPath` exactly) -
  a different, already-verified component and code path, not `BeanVisualizer`'s gizmos (which has
  zero file I/O and could never have produced a PNG). Retried live in `little wings` (a mechanically
  different environment from every prior `project 2` attempt) at the user's suggestion: MCP
  re-probed fresh and confirmed attached there; first framing attempt used a guessed altitude that
  landed inside the terrain (a green textureless close-up, a test-setup mistake, not a UTI finding);
  corrected using the real `PlayerPlane` object's actual position as a reference. Final capture
  confirmed `SceneView.pivot` at the exact requested coordinates, `drawGizmos=true`, and the real
  `PlayerPlane` rendering correctly and live in frame - but still no gizmo line anywhere. This is now
  cross-project evidence (`project 2` and `little wings`) that the capture tooling itself doesn't
  render the Editor Gizmo layer, a sharper finding than "sometimes stale." T05 remains Planned - no
  regression to chase, but still unresolved. See T05's row for full detail.
- 2026-08-09 10:19 — **T05 fourth attempt, still inconclusive - but the failure mode narrowed further.**
  A live Unity MCP connection to `project 2` was probed fresh (per standing instruction), confirmed
  capable of real script execution again. Deliberately sidestepped attempt 3's blocker (`project 2`'s
  own `GameManager` resetting live Play Mode state) by building a real 40-sample path entirely in
  Edit Mode via `BeanTracker`'s public API (`StartTracking()`/`SimulateFrame()`) - `OnDrawGizmos`
  fires in the Editor regardless of Play state, so this needed no Play Mode at all. Confirmed via
  direct API queries (not assumed) that `SceneView.drawGizmos` was on and `SceneView.pivot` was
  framed exactly on the path's bounds, with real level geometry visible at the right scale in two
  live captures - but no gizmo line ever appeared, and a control test (a built-in-icon marker at the
  same point) didn't either. Two `Unity_Camera_Capture` calls taken after a confirmed scene change
  came back byte-identical, revealing stale/cached output as at least part of the cause - the same
  signature as attempt 1. Test objects cleaned up, 0 console errors. Also incidentally reconfirmed
  the `GetInstanceID()`/CS0619 obsolete-as-error finding from a much earlier session, previously
  unconfirmable, now directly hit and worked around the same way (avoid it, no behavior change
  needed in UTI itself since nothing in `Runtime/` calls it - see `BeanArtifactPaths.NewUniqueToken()`).
