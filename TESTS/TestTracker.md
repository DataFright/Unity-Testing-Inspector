...
| T07 | Package | UTI installs as a package in all three test projects | Add a `testables` entry + a dependency in each project's `manifest.json` — **originally tested via `"file:"`, policy changed 2026-08-21 to a tag-pinned GitHub URL instead (see `DESIGN.md` Target Environment table)** | Package resolves, compiles, `UTI` namespace usable | little wings, project 2 | **Pass, but re-scope needed.** Both passed under the old `file:` method (2026-08-06). `little wings` has since re-confirmed under the *new* tag-pinned method too (2026-08-21, `v0.2.0`, as part of BUG-05's verification). `project 2` still needs re-confirming under the new method; `2d project 3` still Planned under either. | 2026-08-06, re-scoped 2026-08-21 |
...
| T21 | BeanMouseTracker | Screen/world mouse-position resolution (EditMode) + live `Update()` behavior (PlayMode) | Run `BeanMouseTrackerTests` (3 EditMode tests) + `BeanMouseTrackerPlayModeTests` (`TESTS/PlayMode/`, covers BUG-05's fix: never-throws regardless of input mode, plus a mode-specific test gated by the same `ENABLE_LEGACY_INPUT_MANAGER` symbol the production code branches on) | All pass | little wings | **Pass — both branches, closed 2026-08-21.** EditMode half Pass since 2026-08-07. PlayMode New-Input-System branch run live via `TestRunnerApi` (console-evidence-confirmed, detail in `BugTracker.md` BUG-05). PlayMode legacy/Both branch confirmed directly by the `little wings` team: flipped Active Input Handling to "Both," added a live `BeanMouseTracker`, clean console in Play Mode, and read the component's own `warnedLegacyInputUnavailable` field via reflection (`false`) to confirm the legacy path itself ran without ever tripping its fallback — not just "nothing looked broken." Setting reverted and reconfirmed on disk afterward. **Forward-looking note from that report:** no public Unity scripting API exists for Active Input Handling (`PlayerSettings.GetSerializedObject()` is `internal`) — the team reached it via reflection into that internal method, then the public `SerializedObject`/`SerializedProperty` API from there. Verified by reading the raw `activeInputHandler` value in `ProjectSettings.asset` directly, not just trusting the call. If this branch ever needs routine regression coverage (rather than a one-off manual check), that reflection trick is worth wrapping into a small editor-only test utility inside UTI itself instead of being reflected into ad hoc each time. | 2026-08-07 (EditMode), 2026-08-20 (PlayMode written), 2026-08-21 (both PlayMode branches verified, closed) |
...
Environment table for why. Swap in whatever the current release tag is (`v0.2.0` as of this
writing):
...
- 2026-08-21 20:25 — T21 CLOSED — both PlayMode branches now verified live. The `little wings`
  team confirmed the legacy/Both-mode branch directly (clean console + reflection-confirmed
  internal state, not just "nothing looked broken"), closing the gap this session's own testing
  left open. Full detail: `BugTracker.md` BUG-05.
